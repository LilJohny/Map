...
def create_map(condition_type, condition, map_file, second_layer_type, limit):
    base_file = os.sep.join([str(Path(os.getcwd()).parent), 'static_files', 'locations.csv'])
    ...
    add_info = list(map(lambda x: x.replace('{', '').replace('}', '').replace('NO DATA', ''), add_info))
    coordinates = get_coordinates(locations)
    locations_map = folium.Map(zoom_start=10)

    if second_layer_type == 1:
        populations_file = os.sep.join([str(Path(os.getcwd()).parent), 'static_files', 'world.json'])
